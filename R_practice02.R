...
welfare<-left_join(welfare, myregion, id='code_region')
...
table(is.na(welfare$code_region)) # 지역정보 결측값 없음
...
table(is.na(mpg$drv)) # drv 변수에 결측치 없음
table(is.na(mpg$hwy)) # hwy 변수에 결측치 5개
